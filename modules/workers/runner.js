...
function prepareTable(name, db, cols) {
  ...
  cols.forEach((col) => {
    if (col === 'id' || col === 'datetime') {
      table.columns.add(col, db.NVarChar(250), {
        nullable: false,
        primary: true
      });
    } else {
      table.columns.add(col, db.NVarChar(db.MAX), {
        nullable: true
      });
    }
  });
...
function makeGenerator({
  ...
  return function* gen() {
    ...
    while (todo.length) {
      ...
      if (true) process.stdout.write(`${message} ${item.name}`);
    ...
    if (true) console.log(`Done: ${origin}_${list}`);
    connection.close();
  };
}
  ...
  connection
    ...
    .then(() => {
      ...
      const pClusters = new database.Request()
        ...
        FROM ${tableName}
        WHERE DATEDIFF(day, [datetime], '${datetime}') <= ${7}`);
